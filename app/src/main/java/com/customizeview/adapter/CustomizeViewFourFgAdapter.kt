...
import com.customizeview.fragment.CustomizeViewFourFragment
...
class CustomizeViewFourFgAdapter : FragmentStatePagerAdapter{

    private var mTitles : ArrayList<String> = ArrayList()
    private var mFragments : ArrayList<CustomizeViewFourFragment> = ArrayList()
    ...
    fun destory(){
        if (mFragments.isNotEmpty()){
            for (fragment: CustomizeViewFourFragment in mFragments){
                fragment.onDestroy()
            }
        }
    }
    ...
    fun add(item : String){
        ...
        mFragments.add(CustomizeViewFourFragment().newInstance(item))
    }
